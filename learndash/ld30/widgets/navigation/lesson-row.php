...
<div class="<?php echo esc_attr($lesson_class); ?>">
    <div class="ld-lesson-item-preview">
        <a class="ld-lesson-item-preview-heading ld-primary-color-hover"
           href="<?php echo esc_url(learndash_get_step_permalink($lesson['post']->ID, $course_id)); ?>">
            ...
            if (function_exists('samadhan_rise_progress_translate')) {


                $progress = samadhan_rise_progress_translate($course_id, [],$lesson['post']->ID);
                $totalProgress = $progress["percentage"];
                $course_progress = $progress['course_progress'];

                $lesson_progress = learndash_get_course_progress( $user_id, $lesson['post']->ID, $course_id );
                $lesson_completed=$lesson_progress['posts'][ $lesson['post']->ID]->completed;

                if (isset($progress["percentage"]) && $totalProgress !== 100 && !$lesson_completed) {

                    $progress_percentage = !empty($course_progress)? ($course_progress[0]/10):0;
                    smdn_custom_progress_bar($progress_percentage);
            ...
            <div class="ld-lesson-title">
                ...
                if (!empty($attributes)) :
                    foreach ($attributes as $attribute) :
                        ?>
                        <span class="ld-status-icon <?php echo esc_attr($attribute['class']); ?>"
                              data-ld-tooltip="<?php echo esc_attr($attribute['label']); ?>"><span
                                    class="ld-icon <?php echo esc_attr($attribute['icon']); ?>"></span></span>
                    <?php
                    endforeach;
                endif;
                ?>
    ...
    <?php if ($expandable) : ?>
        <div class="ld-lesson-item-expanded ld-expandable <?php echo esc_attr('ld-nav-content-list-' . $lesson['post']->ID); ?> <?php echo esc_attr($expand_class); ?>"
             id="<?php echo esc_attr('ld-nav-content-list-' . $lesson['post']->ID); ?>"
             data-ld-expand-id="<?php echo esc_attr('ld-nav-content-list-' . $lesson['post']->ID); ?>">
